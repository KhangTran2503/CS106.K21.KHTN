...
""" check empty cell"""
def isemptycell(layoutChar):
    return layoutChar == ' '

""" check food cell """
def isfoodcell(layoutChar):
    return layoutChar == '.'    
...
def getLayout(name, back = 2):
    """ Modify Name Map Rand
    """
    pos = name.find("Rand")
    isRand = False
    if pos != -1:
        isRand = True
        name = name[:pos] + name[pos + 4:]

    if name.endswith('.lay'):
        layout = tryToLoad('layouts/' + name,isRand)
        if layout == None: layout = tryToLoad(name,isRand)
    else:
        layout = tryToLoad('layouts/' + name + '.lay',isRand)
        if layout == None: layout = tryToLoad(name + '.lay',isRand)
...
def tryToLoad(fullname,isRand):
    ...
    f = open(fullname)
    try:
        return Layout([line.strip() for line in f],isRand)
    finally: f.close()


""" Command with 'name' + "Rand" """ 